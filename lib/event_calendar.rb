module EventCalendar
  ...
  module ClassMethods
    ...
    def events_for_date_range(start_d, end_d, find_options = {})
      events = self.where("('#{start_d.to_time.utc}' <= #{self.quoted_table_name}.#{self.end_at_field}) AND (#{self.quoted_table_name}.#{self.start_at_field}< '#{end_d.to_time.utc}')").order("#{self.quoted_table_name}.#{self.start_at_field} ASC")
      events.where(find_options[:conditions]) if find_options[:conditions]
      events.order(find_options[:order]) if find_options[:order]
      return events.to_a
    end
